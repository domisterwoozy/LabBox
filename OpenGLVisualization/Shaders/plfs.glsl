...
vec3 ApplyLight(vec4 lightPos, vec3 lightColor, float lightPower, float attenuationCoef, vec3 coneDir, float coneAngle, float ambientCoef, vec3 surfaceColor, vec3 normal, vec3 surfacePos, vec3 surfaceToCamera)
{
    vec3 surfaceToLight;
...
void main() {
	...
	{
		linearColor += ApplyLight(lightPositions[i], lightColors[i], lightPowers[i], lightAttenuations[i], coneDirections[i], coneAngles[i], ambientIntensities[i], fragColor.rgb, normal, surfacePos, surfaceToCamera);
	}
